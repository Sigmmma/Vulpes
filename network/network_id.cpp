...
CodeSignature sig_message_delta_object_index(true, std::vector<int16_t>({0x8B, 0x44, 0x24, 0x14, 0x50, 0x8B, 0xCF, 0xB8, -1, -1, -1, -1, 0xE8}) );

CodeSignature sig_func_server_register_network_index(true, std::vector<int16_t>({0x55, 0x56, 0x8B, 0x70, 0x58, 0x8A, 0x46, 0x0C, 0x8D, 0x6E, 0x0C, 0x57, 0x83, 0xCF, 0xFF, 0x3C, 0x01, 0x75}) );

CodeSignature sig_func_client_register_network_index_from_remote(true, std::vector<int16_t>({0x8B, 0x40, 0x58, 0x8B, 0x50, 0x28, 0x53, 0x55, 0x8B, 0x6C, 0x24, 0x0C, 0x57, 0x8D, 0x3C, 0xAA, 0x8B, 0x17, 0x32, 0xDB, 0x83, 0xFA, 0xFF, 0x75}) );

CodeSignature sig_func_unregister_network_index(true, std::vector<int16_t>({0x53, 0x57, 0x8B, 0x78, 0x58, 0x8A, 0x47, 0x0C, 0x8D, 0x4F, 0x0C, 0x32, 0xDB, 0x3C, 0x01, 0x75}) );

CodeSignature sig_translation_table_ptr(true, std::vector<int16_t>({-1, -1, -1, -1, 0x8B, 0x52, 0x28, 0x8B, 0x34, 0x8A}) );
...
typedef struct{
public:
    uint32_t max_count = 511;
    uint32_t int1 = 128;            //0x4
    uint32_t int2;                  //0x8
